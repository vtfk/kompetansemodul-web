<script>
    ...
    export let displayNumberOfItems = 8
    ...
    const filterPreviewData = () => {
        previewData = mapDataList(dataList.filter(ele => filterFunction(inputValue, ele)))
    }
    ...
    {#if focusing && showPreview && (previewData.length > 0)}
        ...
        {#each previewData as pv}
            <div id={pv.id} class="previewItem item{ pv.active ? ' active' : ''}" on:click={pv.onClick}>
                <div class="previewItemProp">{pv.value || '???'}</div><div class="previewItemProp">{pv.second || ''}</div><div class="previewItemProp">{pv.third || ''}</div>
            </div>
        {/each}
        </div>
    {/if}
