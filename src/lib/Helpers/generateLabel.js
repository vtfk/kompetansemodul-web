export function getLabel (total, percentage, county, countyStats) {
    ...
    return (((countyStats / total ) * 100) >= percentage ? `${county} (${countyStats})` : `${county} (${countyStats},  ${((countyStats/ total ) * 100) ===  Infinity ? '0%': Number.isNaN((countyStats / total ) * 100) ? 0 : ((countyStats / total ) * 100).toFixed(1)}%)`)
}