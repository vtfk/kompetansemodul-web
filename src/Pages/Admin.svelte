		...
		{:then res}
			...
			<div class="mailBox">
				...
				{:else if mailStatus.remind.result.length > 0}
					...
					{/if}
				{:else}
					<Button disabled={false} removeSlots={true} buttonText="📧 Send påminnelse til de som ikke har fylt ut" onClick={ () => { sendEmails({ receivers: overview.hasNotAnsweredAll.map(emp => emp.userPrincipalName), template: remindMail }, 'remind') } } />
